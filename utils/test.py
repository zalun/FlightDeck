...
class TestCase(_TestCase):
    ...
    def createCore(self):
        ...
        for sdk in sdks:
            if sdk != '__init__.py':
                sdk_inf = os.stat(os.path.join(settings.FRAMEWORK_PATH, 'lib',sdk))
                if sdk_time < 0 or sdk_time > sdk_inf.st_ctime:
                    sdk_time = sdk_inf.st_ctime
                    self.sdk_filename = sdk
        self.sdk_path = os.path.join(settings.FRAMEWORK_PATH, 'lib/jetpack-sdk')
        sdk_orig = os.path.join(settings.FRAMEWORK_PATH, 'lib', self.sdk_filename)
