...
var dbug = {
	...
	cookie: function(set){
		...
		if((set != null && debugCookie != 'true') || (set)) {
			dbug.enable();
...
/*  	these are the defaults for Element.position anyway
	...
	attach: function(attach){
		var method = [attach, true].pick() ? 'addEvents' : 'removeEvents';
		var events = {};
...
StickyWin = Class.refactor(StickyWin, {
	...
	makeDraggable: function(){
		...
		if (this.options.useIframeShim) {
			this.makeIframeShim();
			var onComplete = (this.options.dragOptions.onComplete || function() { });
	...
	makeResizable: function(){
		...
		if (this.options.useIframeShim) {
			this.makeIframeShim();
			var onComplete = (this.options.resizeOptions.onComplete || function() { });
	...
	toggleVisible: function(show){
		if (!this.visible && (Browser.safari||Browser.chrome) && [show, true].pick()) {
			this.win.setStyles({
				display: 'block',
				opacity: 0
			});
			return true;
		} else if (![show, false].pick()){
			this.win.setStyles({
				display: 'none',
				opacity: 1
			});
...
/*	These options are deprecated:
	...
	initialize: function() {
		...
		if (Browser.ie6) css = css.replace(/png/g, 'gif');
